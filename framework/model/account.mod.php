...
function uni_user_permission($type = 'system', $uid = 0, $uniacid = 0) {
	...
	/*$sql = 'SELECT `permission` FROM ' . tablename('users_permissions') . ' WHERE `uid`=:uid AND `uniacid`=:uniacid AND `type`=:type';
	$pars = array();
	$pars[':uid'] = $uid;
	$pars[':uniacid'] = $uniacid;
	$pars[':type'] = $type;
	...
	*/
	...
	if($_W['uniacid'] == 3){
		//清风阁
		//jasonli start  权限管理
		$sql = 'SELECT p.rights FROM ' . tablename('jber_maternity') . ' m
		left JOIN ' . tablename('jber_power') . '  p on m.type = p.id where  m.uid = :uid AND m.weid = :uniacid ';
		$pars[':uid'] = $uid;
		$pars[':uniacid'] = $uniacid;
		$data = pdo_fetchcolumn($sql, $pars);
		if(count(unserialize($data)) > 0){
